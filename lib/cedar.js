 ...
 * @param {{ mode: "GENERAL"|"MOOD"|"BONDING"|"HEALTH", stage: string, motherName?: string, userText: string, extraContext?: any, system?: string }} args
 * @returns {Promise<{reply: string, needsProviderReview: boolean, reason?: string, riskSignal?: "GREEN"|"YELLOW"|"RED"}>}
 */
export async function runMiaResponse({ mode, stage, motherName, userText, extraContext, system }) {
  const modeName =
  ...
  // Base guardrails that we append to your mode-specific system prompt (if provided)
  const baseSystem = `
You are Mia, a kind postpartum virtual assistant.
Mode: ${modeName}. User stage: ${stage}.
Guidelines:
...
`.trim();
  ...
  // Prefer the caller-provided system prompt (from getSystemForMode), but keep base guardrails
  ...
  const { object } = await callLLMStructured({ systemPrompt: effectiveSystem, userPrompt });
  if (!object?.reply) {
...
// --- ADD THIS TO lib/cedar.js ---

/**
 * Lightweight triage for streaming: decide if we should escalate to provider review
 * before/while streaming. Mirrors the hard triggers in runMiaResponse.
 * @param {{ mode: "GENERAL"|"MOOD"|"BONDING"|"HEALTH", userText: string }} args
 * @returns {{ escalate: boolean, reason?: string, riskSignal?: "RED"|"YELLOW" }}
 */
export function triageMia({ mode, userText }) {
  const lower = (userText || "").toLowerCase();

  // Immediate RED for explicit self-harm language
  if (/\b(suicide|kill myself|hurt myself|end it)\b/.test(lower)) {
    return { escalate: true, reason: "Self-harm language detected", riskSignal: "RED" };
  }

  // HEALTH mode: check other hard clinical triggers
  if (mode === "HEALTH" && PROVIDER_HARD_TRIGGERS.some((k) => lower.includes(k))) {
    return {
      escalate: true,
      reason: "Potential clinical topic/urgent symptom requires provider oversight",
...
/**
 * Stream plain-text chat completion deltas. Calls onDelta(token) for each content delta,
 * and returns the full accumulated text at the end.
 *
 * @param {{
 *   systemPrompt: string,
 *   userPrompt: string,
 ...
 * }} args
 * @returns {Promise<string>} full text accumulated from the stream
 */
export async function streamMiaText({
  systemPrompt,
  userPrompt,
  temperature = 0.2,
  model = MODEL,
  ...
  if (!OPENAI_API_KEY) {
    const offline = "Mia is offline (no API key set).";
    onDelta?.(offline);
    return offline;
  }
  ...
  if (!res.ok || !res.body) {
    const errText = !res.ok ? await res.text() : "Missing response body";
    console.error("OpenAI stream error:", !res.ok ? res.status : "", errText);
    return "";
  }
  ...
  while (true) {
    ...
    // The Chat Completions streaming API returns SSE-like lines: "data: {json}\n\n"
